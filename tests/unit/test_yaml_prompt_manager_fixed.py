...
class TestYAMLPromptManagerFixed:
    ...
    def test_list_templates(self):
        ...
        with patch.object(YAMLPromptManager, "load_configuration"):
            with patch.object(YAMLPromptManager, "load_all_prompts"):
                ...
                manager.error_prompts = {"error1": {"prompt": "Error 1"}}
    ...
    def test_get_template_usage_summary(self):
        ...
        with patch.object(YAMLPromptManager, "load_configuration"):
            with patch.object(YAMLPromptManager, "load_all_prompts"):
                manager = YAMLPromptManager()
                manager.template_usage_count = {"template1": 5, "template2": 3}
    ...
    def test_load_configuration_yaml_error(self, mock_open_file, mock_yaml_load, caplog):
        ...
        with patch.object(Path, "exists", return_value=True):
            ...
            with (
                patch.object(YAMLPromptManager, "load_all_prompts"),
                caplog.at_level(logging.ERROR, logger="yaml_prompt_manager"),
            ):
                manager = YAMLPromptManager()
