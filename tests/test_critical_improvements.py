...
class TestCustomExceptions:
    ...
    def test_ollama_connection_error_with_context(self):
        ...
        error = OllamaConnectionError("Connection failed", host="127.0.0.1", port=11434)
    ...
    def test_ollama_timeout_error_with_context(self):
        ...
        error = OllamaTimeoutError("Request timed out", timeout=600)
    ...
    def test_ollama_model_not_found_error_with_context(self):
        ...
        error = OllamaModelNotFoundError("Model not found", model="llama3.1:8b")
    ...
    def test_reqif_parsing_error_with_context(self):
        ...
        error = REQIFParsingError("Parsing failed", file_path="/path/to/file.reqifz")
    ...
    async def test_async_connection_error_raises_ollama_connection_error(self):
        ...
        async with AsyncOllamaClient(config.ollama) as client:
            with patch.object(client.session, "post") as mock_post:
                ...
                mock_post.side_effect = ClientConnectorError(connection_key=None, os_error=os_error)
...
class TestConcurrentBatchProcessing:
    ...
    async def test_hp_processor_processes_all_requirements_concurrently(self):
        ...
        with (
            patch.object(HighPerformanceREQIFZFileProcessor, "_extract_artifacts") as mock_extract,
            patch.object(
                HighPerformanceREQIFZFileProcessor, "_build_augmented_requirements"
            ) as mock_augment,
            patch.object(
                HighPerformanceREQIFZFileProcessor, "_generate_output_path"
            ) as mock_output,
            ...
            mock_augmented = [
                {"id": f"REQ_{i:03d}", "heading": "Test", "info_list": [], "interface_list": []}
                for i in range(10)
            ]
...
class TestContextAwareProcessingIntegrity:
    ...
    def test_base_processor_context_aware_logic_preserved(self):
        ...
        processor.extractor.classify_artifacts = Mock(
            return_value={
                "System Interface": [create_test_interface("CAN Signal", interface_id="IF_001")]
            }
        )
        ...
        assert "Door Control System" in req1["heading"], (
            "First requirement should have first heading (XHTML format)"
        )
        ...
        assert "Window Control System" in req2["heading"], (
            "Second requirement should have second heading (XHTML format)"
        )
    ...
    def test_context_reset_after_each_requirement(self):
        ...
        assert len(req2["info_list"]) == 1, (
            "REQ_002 should have 1 info item (context reset working)"
        )
        assert req2["info_list"][0]["id"] == "I_002", (
            "REQ_002 should only have info AFTER first req"
        )
...
class TestProcessorExceptionHandling:
    ...
    def test_standard_processor_handles_connection_error(self):
        ...
        with patch(
            "processors.standard_processor.REQIFZFileProcessor._initialize_logger",
            return_value=None,
        ):
            ...
            with patch.object(
                REQIFZFileProcessor,
                "_extract_artifacts",
                side_effect=OllamaConnectionError("Connection failed", "127.0.0.1", 11434),
            ):
                result = processor.process_file(Path("/fake/file.reqifz"), model="llama3.1:8b")
    ...
    def test_standard_processor_handles_model_not_found(self):
        ...
        with patch(
            ...
            with patch.object(
                REQIFZFileProcessor,
                "_extract_artifacts",
                side_effect=OllamaModelNotFoundError("Model not found", "fake:model"),
            ):
                result = processor.process_file(Path("/fake/file.reqifz"), model="fake:model")
