...
class TestPerformanceRegressionBenchmarks:
    ...
    def test_standard_processor_performance_regression(self, temp_reqifz_file, tmp_path, benchmark):
        ...
        with (
            patch(
                "src.processors.base_processor.BaseProcessor._extract_artifacts",
                return_value=[
                    {
            ...
            patch(
                "src.processors.base_processor.BaseProcessor._build_augmented_requirements",
                return_value=(
                    [
                        {
                            "type": "System Requirement",
            ...
            patch(
                "src.core.generators.TestCaseGenerator.generate_test_cases_for_requirement",
                return_value=[
                    {
                        "summary": "Test case",
                        "action": "Test action",
        ...
        assert result["processing_time"] < 5.0, (
            f"Processing took {result['processing_time']:.2f}s, expected < 5.0s"
        )
    ...
    def test_context_aware_processing_performance(self, benchmark):
        ...
        for i in range(25):
            artifacts.append({"type": "Heading", "id": f"HEAD_{i:03d}", "text": f"Chapter {i}"})
        ...
        processor.extractor.classify_artifacts = Mock(
            return_value={"System Interface": [{"type": "System Interface", "text": "CAN Bus"}]}
        )
    ...
    def test_processor_consistency(self, temp_reqifz_file, tmp_path, num_runs):
        ...
        for _ in range(num_runs):
            with (
                patch.object(
                    REQIFZFileProcessor,
                    "_extract_artifacts",
                    return_value=[{"type": "System Requirement", "id": "REQ_001", "table": True}],
                ),
                patch.object(
                    REQIFZFileProcessor,
        ...
        if len(processing_times) > 2:
            ...
            assert time_stdev < (avg_time * 0.5), (
                f"Processing times inconsistent: {processing_times}"
            )
        ...
        with (
            ...
            assert all(
                processor.process_file(temp_reqifz_file, "llama3.1:8b", output_dir=tmp_path)[
                    "success"
                ]
                for _ in range(num_runs)
            )
