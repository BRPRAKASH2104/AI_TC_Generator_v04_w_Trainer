...
class TestTestCaseGenerator:
    ...
    def test_generate_test_cases_with_template(self, mock_ollama_client, sample_requirement, mock_logger):
        ...
        mock_yaml_manager = Mock()
        mock_yaml_manager.get_test_prompt.return_value = {
            "prompt": "Custom template: {requirement_text}",
            "variables": ["requirement_text"]
        }
        ...
        assert result is not None
        mock_yaml_manager.get_test_prompt.assert_called_with("custom_template")
    ...
    def test_prompt_variable_substitution(self, mock_ollama_client, sample_requirement, mock_logger):
        ...
        mock_yaml_manager = Mock()
        mock_yaml_manager.get_test_prompt.return_value = {
            "prompt": "Requirement ID: {requirement_id}, Text: {requirement_text}",
...
class TestAsyncTestCaseGenerator:
    ...
    async def test_generate_test_cases_batch_success(self, mock_async_ollama_client, sample_requirements_list, mock_logger):
        ...
        # Setup async mock
        mock_async_ollama_client.generate_response_async = AsyncMock(
    ...
    async def test_generate_test_cases_batch_with_failures(self, sample_requirements_list, mock_logger):
        ...
        mock_client = Mock()
        async def mock_response(model, prompt):
            if "REQ_002" in prompt:
                raise Exception("AI API timeout")
            return '{"test_cases": [{"summary": "Success"}]}'
        
        mock_client.generate_response_async = AsyncMock(side_effect=mock_response)
        
        mock_yaml_manager = Mock()
        mock_yaml_manager.get_test_prompt.return_value = {
            "prompt": "Generate for {requirement_id}: {requirement_text}",
            "variables": ["requirement_id", "requirement_text"]
        }
        ...
        # Should return results for all requirements, with empty list for failed ones
        assert len(results) == 3
        assert len(results[0]) == 1  # REQ_001 success
        assert len(results[1]) == 0  # REQ_002 failed
        assert len(results[2]) == 1  # REQ_003 success
